#!/usr/bin/env python
"""
Import the QAG Arena Dashboard repository into its dedicated database tables.
This script works specifically with the QAG Arena Dashboard repository and uses separate database tables.
"""
...
from code_monitor.db.models import QAGRepository, QAGRepositoryFile, QAGCodeSnippet
...
class QAGRepositoryImporter:
    """Class to handle importing the QAG Arena Dashboard repository with embeddings"""
    
    def __init__(self):
        self.github_client = GitHubClient()
        self.embedding_generator = EmbeddingGenerator()
        self.code_parser = CodeParser()
        self.qag_repo_url = "https://github.com/statsperform/qag-uat-arena-dashboard"
        
    async def import_repository(self, branch: Optional[str] = None) -> Dict[str, Any]:
        """
        Import the QAG Arena Dashboard repository into dedicated tables
        ...
        """
        logger.info(f"Starting import of QAG Arena Dashboard repository")
    ...
    async def _get_or_create_repository(self, db: AsyncSession, owner: str, name: str, url: str) -> QAGRepository:
        """Get or create a QAG repository record"""
        ...
        logger.info(f"Created new QAG repository record for {owner}/{name}")
        return repository
        
    async def _create_repository_file(self, db: AsyncSession, repository_id: int, file_path: str, 
                                     content: str, language: str) -> QAGRepositoryFile:
        """Create a QAG repository file record with embedding"""
        from sqlalchemy.future import select
        
        # Check if the file already exists for this repository
        query = select(QAGRepositoryFile).where(
            QAGRepositoryFile.repository_id == repository_id,
            QAGRepositoryFile.file_path == file_path
        )
        result = await db.execute(query)
        ...
        if language in ["python", "javascript", "typescript", "java", "c", "cpp", "go", "rust"]:
            try:
                # Extract code snippets using parser
                snippets = await self.code_parser.parse_code(content, language)
                
                # Create code snippets in DB
                for snippet in snippets:
                    code_snippet = QAGCodeSnippet(
                        file_id=repo_file.id,  # Link to the repository file
                    ...
                    snippet_embedding = await self.embedding_generator.generate_embedding(snippet["content"])
                    code_snippet.embedding = snippet_embedding
...
async def main():
    parser = argparse.ArgumentParser(description="Import QAG Arena Dashboard repository into dedicated database tables")
    parser.add_argument("--branch", help="Branch to import (defaults to the default branch)")
    
    args = parser.parse_args()
    
    importer = QAGRepositoryImporter()
    stats = await importer.import_repository(args.branch)
